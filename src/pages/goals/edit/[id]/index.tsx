...
import { useRoqClient, useGoalFindFirst } from 'lib/roq';
...
function GoalEditPage() {
  ...
  const { data, error, isLoading, mutate } = useGoalFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: GoalInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.goal.update({
        data: values as RoqTypes.goal,
        where: {
          id,
        },
      });
