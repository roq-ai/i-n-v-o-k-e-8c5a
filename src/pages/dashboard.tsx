...
import { useRoqPlatformUserProfiles, useGoalCount, useAgentTaskCount, useTaskCount } from 'lib/roq';

import { IntelligentAgentListPage as ShowCaseList } from 'pages/intelligent-agents';

function DashboardPage() {
  const { session } = useSession();

  const goalStat = useGoalCount();

  const agentTaskStat = useAgentTaskCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Goals',
      result: goalStat?.data,
      isLoading: goalStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Agent Tasks',
      result: agentTaskStat?.data,
      isLoading: agentTaskStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Tasks',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Intelligent Agents
                </Text>
                <NextLink href={`intelligent-agents`} passHref legacyBehavior>
                  <Button
