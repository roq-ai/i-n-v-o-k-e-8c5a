...
import { useRoqClient, useIntelligentAgentFindFirst } from 'lib/roq';
...
function IntelligentAgentEditPage() {
  ...
  const { data, error, isLoading, mutate } = useIntelligentAgentFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: IntelligentAgentInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.intelligent_agent.update({
        data: values as RoqTypes.intelligent_agent,
        where: {
          id,
        },
      });
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Intelligent Agent',
              isCurrent: true,
            },
          ]}
        />
